...
if "x%RegDel_SKIP_SOFTWARE%"=="x1" (
  if not "%chk_key%"=="%src_key%" goto :EOF
)

set "chk_key=%src_key:HKLM\Src_SYSTEM\=%"
if "x%RegDel_SKIP_SYSTEM%"=="x1" (
  if not "%chk_key%"=="%src_key%" goto :EOF
)

echo [MACRO]RegDel %*

if "x%~2"=="x" goto :_SimpleDel
set "find_key=%~2"
for /f "delims=" %%A IN ('Reg Query "%src_key%" /s /f "%find_key%"') Do Call :_RegDel "%%A"
goto :EOF

:_SimpleDel
reg delete "%tmp_key%" /f
goto :EOF
